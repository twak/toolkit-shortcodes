(function() {
	tinymce.PluginManager.add('tk-shortcodes', function( editor, url ) {
		...
		function html( cls, data ,con) {
			...
			return '<img data-tk-shortcode="' + sh_tag + '" class="mceItem ' + sh_tag + '" src="' + placeholder + '" ' + 'data-dl-attr="' + data + '" data-dl-content="'+ con+'" data-mce-resize="false" data-mce-placeholder="1" />';
		}
		...
		function restoreShortcodes( content ) {
			return content.replace( /(?:<p(?: [^>]+)?>)*(<img [^>]+>)(?:<\/p>)*/g, function( match, image ) {
				var shortcode = getAttr( image, 'data-tk-shortcode' );
